...
public class DatastreamTableAssessor implements TableAssessor<TableDetail> {
  ...
  public Assessment assess() {
    ...
    if (conf.isUsingExistingStream()) {
      ...
      Stream stream = null;
      try {
        stream = datastream.projects().locations().streams().get(streamPath)
          .execute();
      } catch (IOException e) {
      ...
      try {
        operation = datastream.projects().locations().connectionProfiles()
          .create(parentPath, buildOracleConnectionProfile(oracleProfileName, conf))
      ...
      try {
        streamDeletionOperation =
          datastream.projects().locations().connectionProfiles().delete(streamPath).execute();
      } catch (IOException e) {
      ...
      try {
        sourceProfileDeletionOperation =
          datastream.projects().locations().connectionProfiles().delete(oracleProfilePath).execute();
      } catch (IOException e) {
      ...
      try {
        destinationProfileDeletionOperation =
          datastream.projects().locations().connectionProfiles().delete(gcsProfilePath)
            .execute();
      } catch (IOException e) {
